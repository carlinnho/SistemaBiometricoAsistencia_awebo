...
export default function Padres() {
  ...
  const abrirNuevo = () => {
    setIsEditing(false);
    setPadreEditando(null);
    setOpenModal(true);
  };
  const abrirEditar = (padre) => {
    setIsEditing(true);
    setPadreEditando(padre);
    setOpenModal(true);
  };
  const cerrarModal = () => {
    setOpenModal(false);
  ...
  const handleSave = async (form) => {
    ...
    try {
      ...
      showToast(
        "success",
        `Padre ${isEditing ? "actualizado" : "registrado"} correctamente.`,
      );
      cerrarModal();
      await cargarPadres();
    } catch (err) {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(padresFiltrados.length / ITEMS_PER_PAGE),
  );
  const paginated = padresFiltrados.slice(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              {data.length} padres en el sistema
            </p>
          </div>
        </div>
        <button
        ...
        {isLoading ? (
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-100">
              {paginated.map((padre) => {
                const initials =
                  `${padre.nombre?.[0] ?? ""}${padre.apellido?.[0] ?? ""}`.toUpperCase();
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-9 h-9 rounded-xl ${avatarColor} flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-sm`}
                        >
                          {initials}
                        </div>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full ${padre.activo ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-500"}`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${padre.activo ? "bg-emerald-500" : "bg-red-400"}`}
                        />
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => setPadreAEliminar(padre)}
        ...
        {!isLoading && padresFiltrados.length > 0 && (
          <div className="px-6 py-4 border-t border-slate-100 grid grid-cols-3 items-center">
            ...
            <p className="text-xs text-slate-400">
              ...
              {Math.min(page * ITEMS_PER_PAGE, padresFiltrados.length)} de{" "}
              {padresFiltrados.length} padre
              {padresFiltrados.length !== 1 ? "s" : ""}
            </p>
            ...
            <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-sm shadow-2xl p-7 text-center">
              ...
              <p className="text-sm text-slate-500 mb-6 leading-relaxed">
                ...
                <span className="text-slate-700 font-semibold">
                  {padreAEliminar.nombre} {padreAEliminar.apellido}
                </span>
                . Esta acción es irreversible.
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setPadreAEliminar(null)}
                  disabled={isDeleting}
