...
function Toast({ toast, onClose }) {
  ...
  return createPortal(
    <div
      className={`fixed bottom-5 right-5 z-200 flex items-start gap-3 px-4 py-3 rounded-2xl shadow-2xl text-sm max-w-sm border ${isError ? "bg-red-50 border-red-200 text-red-700" : "bg-emerald-50 border-emerald-200 text-emerald-700"}`}
    >
      {isError ? (
...
const inp =
  "w-full px-4 py-2.5 bg-white border border-slate-200 text-slate-800 placeholder-slate-400 rounded-xl focus:outline-none focus:ring-2 focus:ring-orange-400/50 focus:border-orange-400 transition-colors text-sm";
...
function EditModal({ docente, onClose, onSave }) {
  ...
  // ─── CORRECCIÓN DE CÁMARA Y MODELOS ───
  useEffect(() => {
    ...
    // 3. Limpiamos la cámara al cerrar el modal
    return () => {
      if (activeStream) activeStream.getTracks().forEach((t) => t.stop());
    };
  }, []);

  // 4. Asignamos el video SOLO cuando la etiqueta <video> ya existe
  useEffect(() => {
      ...
      <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/2 p-8 flex flex-col items-center bg-orange-50/40">
              ...
              <>
                ...
                {descriptor && (
                  <div className="absolute inset-0 bg-emerald-500/20 flex items-center justify-center backdrop-blur-sm">
                    <div className="bg-emerald-500 text-white rounded-full p-3 shadow-lg ring-4 ring-emerald-200">
          ...
          {descriptor ? (
            <div className="w-full text-center p-3 bg-emerald-50 rounded-2xl border border-emerald-200 text-emerald-600 text-sm font-medium mb-4">
              ...
              <button
                ...
                className="block w-full mt-1 text-xs text-emerald-500 hover:text-emerald-700 underline"
              >
                Volver a capturar
              </button>
            </div>
            ...
            >
              ...
              {isScanning ? "Analizando..." : "Actualizar Rostro (Opcional)"}
            </button>
          )}
          ...
          <div className="w-full flex gap-3 mt-auto">
            <button
              onClick={onClose}
              className="flex-1 py-2.5 border border-slate-200 text-slate-500 hover:bg-slate-100 rounded-2xl text-sm font-semibold transition-colors"
            >
              Cancelar
            </button>
            <button
...
function NuevoModal({ onClose, onSaved }) {
  ...
  // ─── CORRECCIÓN DE CÁMARA Y MODELOS AQUÍ TAMBIÉN ───
  useEffect(() => {
    let activeStream = null;
  ...
  const handleSave = async () => {
    ...
    if (!descriptor) return alert("Debe escanear el rostro del docente");
    setIsSaving(true);
    try {
      const u = await UsuariosService.create({
        nombre: form.nombre,
      ...
      await HorariosPersonalService.create({
        id_usuario: u.id,
  ...
  const cargarDocentes = useCallback(async () => {
    ...
    try {
      ...
      setData(
        docentes.map((d) => ({
          ...d,
          horario: horarios.find((h) => h.id_usuario === d.id),
        })),
      );
    } catch (err) {
  ...
  const handleCardSave = async (docente, form, descriptor) => {
    try {
      ...
      if (docente.horario?.id_horario_usuario) {
        await HorariosPersonalService.update(
          docente.horario.id_horario_usuario,
          {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              {data.length} docentes en el sistema
            </p>
          </div>
        </div>
        <button
      ...
      {!isLoading && docentesFiltrados.length > 0 && (
        <p className="mt-6 text-xs text-slate-400">
          {docentesFiltrados.length} resultado
          {docentesFiltrados.length !== 1 ? "s" : ""}
        </p>
      )}
      ...
      {docenteEditando && (
        <EditModal
          ...
          onSave={async (docente, form, descriptor) => {
            await handleCardSave(docente, form, descriptor);
          }}
        />
      )}
